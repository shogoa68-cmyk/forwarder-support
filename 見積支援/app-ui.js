// ========== UI・共通 (app-ui.js) ==========
  ...
  async function doFetchFxRates() {
    ...
    showToast(ok ? '✅ 為替レートを更新しました' : '⚠️ 取得失敗。デフォルト値を使用します', ok ? 'success' : 'warn', 3000);
  }
  ...
  function doAddUserCat() {
    ...
    if (!label) { showToast('⚠️ カテゴリ名を入力してください', 'warn'); return; }
    const cats = getUserCategories();
    if (cats.some(c => c.label === label)) { showToast('⚠️ 同名のカテゴリが既にあります', 'warn'); return; }
    ...
    showToast(`✅ カテゴリ「${label}」を追加しました`, 'success');
  }
  ...
  function doDeleteUserCat(value) {
    ...
    renderUserCatPanel();
    refreshAllCategoryDropdowns();
    showToast(`🗑️ 「${cat.label}」を削除しました`, 'info');
  }
  ...
  function copySelectedRows() {
    ...
    if (!checkboxes.length) {
      showToast('⚠️ コピーしたい行のチェックボックスにチェックを入れてください', 'warn', 3000);
      return;
    }
    ...
    updateTotals();
    showToast(`📋 ${srcRows.length}行をコピーしました`, 'success');
  }
  ...
  function deleteSelectedRows() {
    const checkboxes = document.querySelectorAll('.row-select-chk:checked');
    if (!checkboxes.length) {
      showToast('⚠️ 削除したい行のチェックボックスにチェックを入れてください', 'warn', 3000);
      return;
    }
    ...
    if (allChk) allChk.checked = false;
    updateTotals();
    showToast(`🗑️ ${rows.length}行を削除しました`, 'info');
  }
  ...
  function changeFontSize(delta) {
    ...
    showToast(`🔠 文字サイズ: ${next}px`, 'info', 1500);
  }
  ...
  // DOMContentLoaded後に並び替え初期化・順序復元
