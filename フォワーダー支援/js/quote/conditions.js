// ========== 引き合い条件・ゾーン (app-conditions.js) ==========
  ...
  const ROW_CELL_FIELDS = ['cat','sv','tx','nm','pq','un','bq','pc','bc','pp','bp','cd','mk','nt','zc','vf','vt','ac','pt','ps','co','lu','ppmode','pprate','ppbase','uid','ppref','ri','es'];
  ...
  // 1パターン分の明細行を読みやすい文字列に（「荷姿 × 個数（寸法、重量、段積み）」を改行区切り。
  // 同じパターン内に複数の荷姿明細行があるとき、／区切りの1行だとPDF/プレビューで
  // セル幅を超えて不格好に折り返されるため、行ごとに改行して1明細＝1行にする）
